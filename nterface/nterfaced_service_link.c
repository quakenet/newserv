...
void connect_link(void *vlink) {
  ...
  if (setsockopt(fd, SOL_SOCKET, SO_KEEPALIVE, (const char *)&opt, sizeof(opt))) {
    nterface_log(ndl, NL_ERROR, "SL: Unable to set SO_KEEPALIVE on socket (%s:%d), retrying in %d seconds.", nlink->hostname->content, nlink->port, RECONNECT_DURATION);
    close(fd);
    link_schedule_reconnect(nlink);
    return;
  }
