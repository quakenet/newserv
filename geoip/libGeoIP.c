...
GeoIP* GeoIP_new (int flags, char *filename) {
	GeoIP * gi;
...
GeoIP* GeoIP_open (const char * filename, int flags) {
	...
	if (gi->GeoIPDatabase == NULL) {
		...
		st_size = ftell(gi->GeoIPDatabase);
		fseek(gi->GeoIPDatabase, 0, SEEK_SET);
		if (flags & GEOIP_MEMORY_CACHE) {
			gi->cache = (unsigned char *) malloc(sizeof(unsigned char) * st_size);
			if (gi->cache != NULL) {
				if (fread(gi->cache, sizeof(unsigned char), st_size, gi->GeoIPDatabase) != st_size) {
...
int GeoIP_country_info_by_id(int id, char **two, char **three, char **full) {
  if(id < 0)
    return 0;
  *two = (char *)GeoIP_country_code[id];
  *three = (char *)GeoIP_country_code3[id];
  *full = (char *)GeoIP_country_name[id];
...
int GeoIP_id_by_ipnum (GeoIP* gi, unsigned long ipnum) {
	if (ipnum == 0)
		return 0;
	return _seek_record(gi, ipnum) - COUNTRY_BEGIN;
}
